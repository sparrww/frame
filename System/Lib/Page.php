...
namespace Lib;
 ...
 class Page extends Lib{
     ...
     * return string
     */
    function __construct($itemtotal, $pageper = 20,  $pagestyle=array(),$showNum='10') {
        $this->showNum = $showNum;
        $this->curpage = @sprintf("%d", $_GET[$this->pagetag]); //获取当前页数
        $this->total = $itemtotal;  //记录总数
        $this->pageper = ($pageper>0) ? $pageper : 20; //每页记录数
...
//            $pageBar = "<form name=\"page_jump\" action=\"$jump_url\" method=\"get\" style=\"width:auto;padding:0;margin:0;\">".$pageBar."<form>";
    ...
    public function geturl($page=null ,$mode = 1) {
		...
		if($server[0]=="Apache"){
			...
			$r=explode("/",$_SERVER['HTTP_X_REWRITE_URL']);
		}

        if($r[1] == 'Admin'){
            if(count($r)>1){
                if(!$_GET['page']){
                    $url=join('/',$r)."page/".$page;
                }else{
                    $p=explode("/",$r[0]);
                    unset($p[(count($p)-1)]);
                    foreach($p as $v){
 ...
 * @return string
 */
// function sqlpage($sql,$row,$item_count=100,$page_per=20){
// 	$page = new page($item_count,$page_per); 
// 	$sql = $page->handlesql($sql);

// 	$array['list']	=	$row;
// 	$array['fy']		=	$page->getpage();
// 	$array['total']	=	$page->gettotal();
// 	return $array;
// }
